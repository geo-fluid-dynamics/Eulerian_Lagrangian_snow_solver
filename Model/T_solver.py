...
def solve_for_T(T, rho_v_dT, k_eff, D_eff, rhoC_eff, phi, nz, dt, dz, Eterms):
    ...
    main_AT[1:-1] = a[1:-1] + dt * 2 * 2 * beta[1:-1] / (
        dz_nz[1:-1] ** 2 + dz_nz[:-2] ** 2
    )
    for i in range(1, nz - 1):  # k+1
        upper_AT[i] = -dt * ((beta[i + 1] - beta[i - 1])) / (
            dz_nz[i] + dz_nz[i - 1]
        ) ** 2 - dt * (2 * beta[i]) / (dz_nz[i] ** 2 + dz_nz[i - 1] ** 2)
    for i in range(0, nz - 2):  # k-1
        lower_AT[i] = dt * (beta[i + 2] - beta[i]) / (
            dz_nz[i + 1] + dz_nz[i]
        ) ** 2 - dt * (2 * beta[i + 1]) / (dz_nz[i + 1] ** 2 + dz_nz[i] ** 2)
    main_AT[0] = 1
    upper_AT[0] = 0
    main_AT[-1] = 1
    lower_AT[-1] = 0
    ...
    AT = (
        np.diag(np.ones(nz) * main_AT, k=0)
        + np.diag(np.ones(nz - 1) * lower_AT, k=-1)
    ...
    ET = np.diag(np.ones(nz - 1) * lower_ET, k=-1) + np.diag(
        np.ones(nz - 1) * upper_ET, k=1
    )
