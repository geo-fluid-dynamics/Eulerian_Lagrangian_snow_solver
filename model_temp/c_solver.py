...
def solve_for_c(T, T_prev, phi, D_eff, rho_v_dT, nz, dt, dz, Eterms):
    ...
    for i in range(1, nz - 1):  # k+1
        upper_Ac[i] = -((beta[i + 1] - beta[i - 1])) / (
            dz_nz[i] + dz_nz[i - 1]
        ) ** 2 - (2 * beta[i]) / (dz_nz[i] ** 2 + dz_nz[i - 1] ** 2)
    for i in range(0, nz - 2):  # k-1
        lower_Ac[i] = (beta[i + 2] - beta[i]) / (dz_nz[i + 1] + dz_nz[i]) ** 2 - (
            2 * beta[i + 1]
    ...
    Ac = (
        np.diag(np.ones(nz) * main_Ac, k=0)
        + np.diag(np.ones(nz - 1) * lower_Ac, k=-1)
    ...
    lower_Ec[-1] = 0
    ### Set up Matrix Ec
    Ec = np.diag(np.ones(nz - 1) * lower_Ec, k=-1) + np.diag(
        np.ones(nz - 1) * upper_Ec, k=1
    )
    # Set up matrix Bc
    main_Bc = -a
